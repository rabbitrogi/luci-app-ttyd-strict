...
## exit 后快速恢复（2026-09-10 下午晚）

终端内敲 exit → shell 退出 → ttyd --once 退出 → 前端显示
"Press 回车 Reconnect" 但端口已无监听，回车无效。跨域 iframe
无法截获回车，改为缩短感知延迟：轮询 10s→3s（仅激活页面），
自动重启节流 15s→5s。实测 exit 后 1.3 秒新会话自动恢复（新
pid + 客户端连上 + iframe 重挂），Reconnect 屏一闪而过。
另：早前测试遗留的 headless Chrome（注入过粘性 focus，轮询不
停重启会话）被用户 kill -9——测试进程必须即时清理，已记教训。

## -m 1 上游行为版本（2026-09-10 傍晚，待 20:04 后终审取舍）

按用户设计评审：外观与行为须与上游完全一致（exit→回车→重连），
自动重连全部禁止。实现：
- ttyd 参数 -o → -m 1（监听器常驻，最多一个 ws 客户端，排他保留）
- exit 后监听器保留（同 pid、无客户端）→ 前端自己的
  "Press Enter to Reconnect" 生效——回车即重连，上游语义
- 自动重启/15s 重挂全拆除；页面附着仅发生在进入/获得焦点且
  尚未挂载时（ours+0 && !mounted → attach）
- pagehide beacon 恢复但 pid 认领（陈旧 beacon 记日志跳过，
  不可能误杀新会话；沙盒 T14）
- 插件看门狗：页面轮询不断续命，页面消失后 WATCHDOG_SECS
  （默认 600s）收掉无客户端监听器（沙盒 T15，崩溃孤儿兜底）
- 沙盒 43/43

E2E（headless+注入 focus）：进入 ours+1 ✓；exit → ours+0 同 pid ✓；
回车 → 同 pid 客户端恢复 ✓（netstat 2×ESTABLISHED 其中 1 条为
HTTP keep-alive，属计数口径非多客户端）；遗留问题：回车重连后
touch 铁证未通过（疑焦点/时序，待查）。

## 待决策：-m 1 vs 回退 --once（用户提出 20:04 后讨论）

--once 优势：连接结束实例即退，零维护（无 beacon/看门狗/孤儿问
题）；劣势：exit 后监听器消失，回车无效。
--once 可行改良案（保留上游回车体验）：poll 检测 none 后仅恢复
监听器、不碰 iframe——旧提示屏保留，用户回车 → 前端重试 → 连
上新实例 → 新 shell。自动的只是"备好监听器"（用户不可见），真
正的重连仍由回车触发，符合"用户主动"原则。

## --once + 恢复监听器方案（2026-09-10 晚，终版方向）

按用户决策回归 --once（断即亡，安全模型最简）：
- exit → ws 断 → ttyd 自退（state=none，实测 1.5s 内）
- 焦点页轮询见 none → 仅恢复监听器（session_start），绝不动
  iframe——回车提示屏保留
- 关键实测发现（ttyd 前端源码 xterm/index.ts 证实）：前端重连
  行为由 ws 关闭码决定——code 1000=等回车；code≠1000（进程退出
  =1006）=立即自动重连循环。--once 的退出必然走自动分支，故
  "恢复监听器后前端自动重连"（实测：未敲键 clients 已恢复）。
  回车驱动的上游体验只有 -m 1（监听器存活，ws 服务端正常关闭
  1000）才能达成——两方案的本质差异就在此。
- 孤儿监听器（恢复后无人回车且页面已关）：心跳文件收尾（每次
  插件调用刷新；无客户端且心跳>60s → 收掉）。EXIT trap 时序防
  自欺。看门狗/beacon 全部删除。
- 沙盒 45/45（T15 改心跳双用例：新鲜保留/过期回收）。
E2E：进入 ✓；exit→none ✓；恢复新 pid+前端自动重连 ✓；iframe
未触碰 ✓；clients≥1 ⟹ 子 shell（ttyd 每客户端必拉命令进程）；
刷新循环在本生命周期机制下此前两轮 5/5。

## 语义定稿：重新进入取代重连（2026-09-10 深夜）

用户洞见：主动刷新=重新点终端菜单=全新开始——不存在需要保真的
"重连"状态。none 分支简化为 ensureSession()（全新会话+全新挂载
iframe），"恢复监听器但不碰 iframe"特例删除，回车截获问题消失。
E2E：exit→none→轮询≤10s 自动重入（新 pid、新挂载）✓✓。
另复盘两起新诊断坑：①luci 构建会压缩 JS（321→22 行），带空格
